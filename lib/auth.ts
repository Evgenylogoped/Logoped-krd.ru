      ...
      if (session.user) {
        ...
        try {
          ...
          if (typeof t2.id === 'string') {
            const u = await prisma.user.findUnique({ where: { id: t2.id }, select: { city: true, image: true, name: true } })
            if (u && (u as { city?: string | null }).city) (su.city = (u as { city?: string | null }).city as string)
          if (u && (u as { image?: string | null }).image) (su.image = (u as { image?: string | null }).image as string)
          if (u && (u as { name?: string | null }).name) (session.user.name = (u as { name?: string | null }).name as string)
          }
        } catch {}
