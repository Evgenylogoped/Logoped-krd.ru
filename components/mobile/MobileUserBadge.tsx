...
function formatSurnameInitials(name?: string | null, email?: string | null) {
  ...
  const initials = parts.slice(1).map(p => (p && (p as string)[0] ? ((p as string)[0].toUpperCase() + '.') : '')).join('')
...
export default function MobileUserBadge() {
  ...
  return (
    <Link
      href="/"
      className="inline-flex items-center gap-2 px-2 py-1 rounded-full border bg-gray-50 text-gray-800 text-xs max-w-[200px]"
