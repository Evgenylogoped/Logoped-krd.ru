...
export async function acceptInvite(formData: FormData) {
  ...
  const leader = await prisma.user.findUnique({ where: { email: req.leaderEmail.toLowerCase() } }) as any
  if (!leader?.branch?.companyId) throw new Error('Компания руководителя не найдена')
  ...
  if (currentLogopeds >= (company.allowedLogopeds || 0)) {
