...
export async function approveParentBooking(formData: FormData): Promise<void> {
  ...
  if (!bookingId) return
  const booking = await (prisma as any).booking.findUnique({ where: { id: bookingId }, include: { lesson: true } })
...
export async function createConsultationRequest(formData: FormData): Promise<void> {
  ...
  const me = await prisma.user.findUnique({ where: { id: supervisorId }, include: { branch: { include: { company: true } } } }) as any
  const subordinate = await prisma.user.findUnique({ where: { id: subordinateId }, include: { branch: true } }) as any
  ...
  if (isOrgLeader && me?.branch?.companyId && subordinate?.branchId) {
    ...
    const rel = await (prisma as any).userSupervisor.findUnique({ where: { supervisorId_subordinateId: { supervisorId, subordinateId } } })
    allowed = Boolean(rel)
  }
  if (!allowed) {
    revalidatePath('/logoped/schedule');
  ...
  const lesson = await (prisma as any).lesson.findUnique({ where: { id: lessonId }, include: { enrolls: true, bookings: true } })
  if (!lesson || lesson.logopedId !== subordinateId) { revalidatePath('/logoped/schedule'); return }
  const hasEnroll = (lesson.enrolls?.length ?? 0) > 0
  const hasActiveBooking = (lesson.bookings || []).some((b: any) => b.status === 'ACTIVE')
  if (hasEnroll || hasActiveBooking) { revalidatePath('/logoped/schedule'); return }
  // create request if not exists for this lesson
  const existing = await (prisma as any).consultationRequest.findFirst({ where: { lessonId, status: 'PENDING' } })
  ...
  if (emailUser && emailUser.role === 'PARENT') {
    ...
    if (parent) {
      ...
      if (!sameChild) {
        ...
        await prisma.child.create({ data: { parentId: parent.id, lastName: childLastName, firstName: childFirstName, isArchived: false } as any })
  ...
  await (prisma as any).consultationRequest.create({ data: { supervisorId, subordinateId, lessonId, parentEmail, childFirstName, childLastName, note } })
...
export async function approveConsultationRequest(formData: FormData): Promise<void> {
  ...
  if (!requestId) return
  const req = await (prisma as any).consultationRequest.findUnique({ where: { id: requestId }, include: { lesson: true } })
  ...
  if (!user) {
    user = await prisma.user.create({ data: { email: parentEmail, role: 'PARENT', passwordHash: '!' } })
  }
  let parent = await prisma.parent.findUnique({ where: { userId: user.id } })
  if (!parent) {
    parent = await prisma.parent.create({ data: { userId: user.id, fullName: undefined, isArchived: false } as any })
  ...
  if (child) {
    ...
    child = await prisma.child.create({ data: { parentId: parent.id, lastName: childLastName, firstName: childFirstName, logopedId: subordinateId, isArchived: false } as any })
  }
  // enroll to lesson
  await prisma.enrollment.create({ data: { childId: child.id, lessonId: req.lessonId, status: 'ENROLLED' } })
  // mark request approved
  await (prisma as any).consultationRequest.update({ where: { id: requestId }, data: { status: 'APPROVED', respondedAt: new Date() } })
  // notify supervisor and parent
  const sup = await (prisma as any).user.findUnique({ where: { id: req.supervisorId } })
  if (sup?.email) await sendMail({ to: sup.email, subject: 'Заявка на консультацию принята', text: 'Подчинённый принял заявку и создана карточка ребёнка.' })
  if (user.email) await sendMail({ to: user.email, subject: 'Создана карточка ребёнка', text: 'Логопед создал карточку и записал на консультацию. Пожалуйста, войдите в личный кабинет.' })
...
export async function rejectConsultationRequest(formData: FormData): Promise<void> {
  ...
  const requestId = String(formData.get('requestId') || '')
  if (!requestId) return
  const req = await (prisma as any).consultationRequest.findUnique({ where: { id: requestId } })
  if (!req || req.subordinateId !== subordinateId || req.status !== 'PENDING') { revalidatePath('/logoped/schedule'); return }
  await (prisma as any).consultationRequest.update({ where: { id: requestId }, data: { status: 'REJECTED', respondedAt: new Date() } })
  const sup2 = await (prisma as any).user.findUnique({ where: { id: req.supervisorId } })
  if (sup2?.email) {
    await sendMail({ to: sup2.email, subject: 'Заявка на консультацию отклонена', text: 'Подчинённый отклонил вашу заявку на консультацию.' })
  }
  revalidatePath('/logoped/schedule')
  redirect('/logoped/schedule?consult=rejected')
}

export async function approveConsultationCreateChild(formData: FormData): Promise<void> {
...
export async function updateScheduleSettings(formData: FormData): Promise<void> {
  ...
  const data: any = { scheduleSlotMinutes: slot, scheduleBreakMinutes: brk }
...
export async function addTemplateInterval(formData: FormData): Promise<void> {
  ...
  await prisma.workTemplate.create({ data: { userId, dayOfWeek, startMinutes: sh * 60 + sm, endMinutes: eh * 60 + em } })
  revalidatePath('/logoped/schedule')
...
async function ensureDefaultGroup(userId: string) {
  ...
  const user = await prisma.user.findUnique({ where: { id: userId }, include: { branch: true } }) as any
...
export async function createBooking(formData: FormData): Promise<void> {
  ...
  const active = lesson.bookings?.find((b: any) => b.status === 'ACTIVE')
  if (active) { revalidatePath('/logoped/schedule'); return }
  await prisma.booking.create({ data: { lessonId, holder, createdBy, ...(childId ? { childId } : {}) } as any })
  revalidatePath('/logoped/schedule')
}
