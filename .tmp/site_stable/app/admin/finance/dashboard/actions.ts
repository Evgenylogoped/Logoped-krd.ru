...
export async function backfillLeaderLinks(): Promise<void> {
  ...
  for (const t of therapists as any[]) {
    ...
    let group = await prisma.group.findFirst({ where: { branchId: t.branchId } })
    if (!group) group = await prisma.group.create({ data: { name: 'General', branchId: t.branchId } })
    const res = await prisma.lesson.updateMany({ where: { logopedId: t.id, group: { branchId: { not: t.branchId } } }, data: { groupId: group.id } })
    lessonsRebound += res.count || 0
  }
  ...
  const txToFix = await (prisma as any).transaction.findMany({ where: { lessonId: { in: Array.from(lessonBranchMap.keys()) }, OR: [{ branchId: null }, { companyId: null }] }, select: { id: true, lessonId: true } })
  let txFixed = 0
