                  ...
                  <div className="space-y-1">
                    ...
                    {(p as any).children?.map((ch: any) => (
                      <div key={ch.id} className="text-xs">
                        ...
                        <span className="ml-2 text-muted">Логопед: {ch.logoped ? (<>{ch.logoped.name ? `${ch.logoped.name} · ` : ''}<a className="link" href={`mailto:${ch.logoped.email}`}>{ch.logoped.email}</a></>) : '—'}</span>
                      </div>
                    ))}
                  </div>
                </td>
