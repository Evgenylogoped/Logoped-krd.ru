    ...
    <div className="container space-y-6 py-6">
      ...
      <section className="section" style={{ background: 'var(--card-bg)' }}>
        ...
        <div className="space-y-2">
          ...
          {incomingOrgMemberships.map((r: any) => (
      ...
      <section className="section" style={{ background: 'var(--card-bg)' }}>
        ...
        <div className="space-y-2">
          {transferIncomingPending.length === 0 && <div className="text-sm text-muted">Нет входящих передач</div>}
          {transferIncomingPending.map((t: any) => (
            <div key={t.id} className="flex flex-col gap-2 p-3 rounded-md border shadow-sm sm:flex-row sm:items-center sm:justify-between" style={{ background: 'var(--card-bg)' }}>
              <div className="text-sm">
                ...
                <div className="text-muted">Ребёнок: {t.child?.lastName} {t.child?.firstName} · от: {t.fromLogoped?.name || t.fromLogoped?.email}</div>
      ...
      <section className="section">
        ...
        <div className="space-y-2">
          {incomingPending.length === 0 && <div className="text-sm text-muted">Нет заявок</div>}
          {incomingPending.map((r: any) => (
