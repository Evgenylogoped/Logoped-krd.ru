...
import { updateChild, uploadMaterial, deleteMaterial, addProgress, deleteProgress, addReward, deleteReward, uploadChildPhoto, updateParent, requestTransfer, approveTransfer, rejectTransfer } from './actions'
...
export default async function LogopedChildPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams?: Promise<{ tab?: string; evalPage?: string; err?: string; saved?: string; evalMonth?: string }> }) {
  const { id } = await params
  const sp = (searchParams ? await searchParams : {}) as { tab?: string; evalPage?: string; err?: string; saved?: string; evalMonth?: string }
  ...
  return (
    <div className="container space-y-6 py-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">{child.lastName} {child.firstName}</h1>
        <Link href="/logoped/clients" className="btn">Назад</Link>
      </div>
      ...
      <nav className="flex gap-2">
        <Link href={`?tab=main`} className={`btn text-sm ${tab==='main'?'btn-secondary':''}`}>Основное</Link>
        <Link href={`?tab=parent`} className={`btn text-sm ${tab==='parent'?'btn-secondary':''}`}>Родитель</Link>
        <Link href={`?tab=history`} className={`btn text-sm ${tab==='history'?'btn-secondary':''}`}>История</Link>
        <Link href={`?tab=materials`} className={`btn text-sm ${tab==='materials'?'btn-secondary':''}`}>Материалы и ДЗ</Link>
        <Link href={`?tab=progress`} className={`btn text-sm ${tab==='progress'?'btn-secondary':''}`}>Прогресс</Link>
        <Link href={`?tab=rewards`} className={`btn text-sm ${tab==='rewards'?'btn-secondary':''}`}>Награды</Link>
      </nav>
      ...
      {tab === 'history' && (
        <section className="section">
          ...
          <div className="space-y-2">
            {child.enrolls.length === 0 && <div className="text-sm text-muted">История отсутствует</div>}
            {child.enrolls.map((e: any) => (
