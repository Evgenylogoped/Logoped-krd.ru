                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted">
                      Родитель: {' '}
                      <LogopedPreviewTrigger name={(ch.parent as any).fullName || ch.parent.user.name || ch.parent.user.email} image={(ch.parent.user as any)?.image} subtitleLines={[((ch.parent as any)?.phone ? `Телефон: ${(ch.parent as any).phone}` : '')]} actionHref={`/chat/${convByActiveChild[ch.id] || ''}`} actionLabel="Написать">
              ...
              <div key={ch.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 gap-3 rounded-md border shadow-sm" style={{ background: 'var(--card-bg)' }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted">
                      Родитель: {' '}
                      <LogopedPreviewTrigger name={(ch.parent as any).fullName || ch.parent.user.name || ch.parent.user.email} image={(ch.parent.user as any)?.image} subtitleLines={[((ch.parent as any)?.phone ? `Телефон: ${(ch.parent as any).phone}` : '')]} actionHref={`/chat/${convByArchivedParent[ch.parent.user.id] || ''}`} actionLabel="Написать">
                        <span className="underline cursor-pointer truncate max-w-[180px] inline-block align-bottom" title={(ch.parent as any).fullName || ch.parent.user.name || ch.parent.user.email}>{(ch.parent as any).fullName || ch.parent.user.name || ''}</span>
                      </LogopedPreviewTrigger>
                    </div>
                  </div>
