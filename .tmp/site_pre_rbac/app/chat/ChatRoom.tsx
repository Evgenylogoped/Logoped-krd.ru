...
export default function ChatRoom({ conversationId, selfId, initialMessages, childInfo, backgroundColor, backgroundUrl }: { conversationId: string; selfId: string; initialMessages: Message[]; childInfo?: { id: string; name: string; image?: string | null }; backgroundColor?: string | null; backgroundUrl?: string | null }) {
    ...
    const body = (
      <>
        ...
        <div className="mt-1 text-[10px] opacity-70 flex items-center justify-end gap-1">
          <span>{new Date(m.createdAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}{m.editedAt ? ' · изменено' : ''}</span>
          {mine && (
            <span aria-label={new Date(m.createdAt).getTime() <= otherReadTs ? 'Прочитано' : 'Отправлено'}>
              {new Date(m.createdAt).getTime() <= otherReadTs ? '✓✓' : '✓'}
            </span>
          )}
        </div>
