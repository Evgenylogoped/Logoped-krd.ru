...
async function ensureSameCompany(userId: string, companyId: string) {
  ...
  export async function createBranch(formData: FormData) {
    ...
    assert(nonMain.length < (company.allowedBranches || 0), 'Достигнут лимит филиалов')
    // автонумерация
  ...
  if (toBranchId && !user.branchId) {
    const company = await prisma.company.findUnique({ where: { id: companyId }, include: { branches: { select: { id: true } } } }) as any
    const currentLogopeds = await prisma.user.count({ where: { role: 'LOGOPED', branch: { companyId } } })
    assert(currentLogopeds < (company.allowedLogopeds || 0), 'Достигнут лимит логопедов в компании')
  }
