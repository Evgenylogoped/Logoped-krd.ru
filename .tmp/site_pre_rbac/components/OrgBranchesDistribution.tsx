...
export default function OrgBranchesDistribution({ days, extraQuery, refreshIntervalMs }: { days: 7|30|90, extraQuery?: string, refreshIntervalMs?: number }) {
  ...
  const exportCsv = () => {
    const rows = [['branch_id','branch_name','lessons'] as (string)[], ...dist.map(d=> [d.id, d.name, String(d.lessons)])]
    const csv = rows.map(r=> r.map(x=>`"${(x||'').replace(/"/g,'""')}"`).join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `branches_distribution_${days}d.csv`
    a.click()
  ...
  return (
    <div className="mt-3">
      <div className="flex items-center gap-2 text-xs text-muted mb-2">
        ...
        <button className="btn btn-ghost btn-xs" onClick={exportCsv}>Экспорт CSV</button>
      </div>
