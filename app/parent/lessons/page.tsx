                    ...
                    <div key={`${r.kind==='enrollment' ? `${e.childId}:${e.lessonId}` : `booking:${e.id}`}`} className={`rounded-md p-2 ${r.kind==='booking' ? 'border-l-4 border-amber-400' : 'border'} border-gray-200`} style={{ background: 'var(--card-bg)' }}> 
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <LogopedPreviewTrigger name={logoped?.name || logoped?.email || 'Логопед'} image={logoped?.image} actionHref={logoped?.id ? `/chat?to=${logoped.id}&child=${e.childId}` : undefined} actionLabel={logoped?.id ? 'Написать' : undefined} phone={logoped?.phone || (logoped as any)?.user?.phone || null} instagram={logoped?.instagram} subscription={(logoped as any)?.plan || (logoped as any)?.subscription || (logoped as any)?.subscriptionPlan || null} isOnline={logoped?.isOnline} isOffline={logoped?.isOffline} lessonPrice={logoped?.lessonPrice} showPriceToParents={logoped?.showPriceToParents} address={logoped?.address} profession={logoped?.profession} experienceYears={logoped?.experienceYears} specialization={logoped?.specialization} about={logoped?.about} education={logoped?.education} hideAboutFromParents={logoped?.hideAboutFromParents} hideEducationFromParents={logoped?.hideEducationFromParents}>
