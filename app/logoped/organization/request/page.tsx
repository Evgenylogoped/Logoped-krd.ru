...
export default async function OrganizationRequestPage({ searchParams }: { searchParams?: Promise<{ sent?: string; err?: string; cancelled?: string }> }) {
  ...
  return (
    <div className="container py-6 max-w-2xl space-y-4">
      ...
      <div className="text-xs text-muted">Администраторы обрабатывают заявки на странице <a className="underline" href="/admin/org-requests" target="_blank">Админ → Заявки</a>.</div>
      {companyName && (
      ...
      {sp?.sent === '1' && !companyName && !isPending && (
