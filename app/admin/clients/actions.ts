...
export async function updateParentAdmin(formData: FormData) {
  ...
  // normalize phone to +7XXXXXXXXXX (or null if invalid/empty)
  ...
  if (!parent) return
  // Если телефон введён, но невалидный — показать явное предупреждение и остаться в режиме редактирования
  if (phoneRaw && !phone) {
    revalidatePath('/admin/clients')
    // @ts-ignore
    ;(await import('next/navigation')).redirect(`/admin/clients?editParent=${encodeURIComponent(parentId)}&phoneError=1`)
    return
  }
