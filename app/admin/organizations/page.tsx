...
import { liquidateCompany, updateCompanyLimits, approveExpansionRequest, rejectExpansionRequest, deleteCompanyNow, assignOrgManager, revokeOrgManager } from './actions'
...
export default async function AdminOrganizationsPage({ searchParams }: { searchParams?: Promise<Record<string, string>> }) {
  ...
  return (
    <div className="container py-6 space-y-4">
      ...
      {((sp as any)?.err === 'quota_branches_free') && (
        <div className="rounded border p-3 bg-red-50 text-red-700 text-sm">
          Превышен лимит филиалов для бесплатного плана. Увеличьте план в разделе
          {' '}<a className="underline" href="/settings/billing">Настройки → Подписка</a>.
        </div>
      )}
      {((sp as any)?.err === 'quota_logopeds_free') && (
        <div className="rounded border p-3 bg-red-50 text-red-700 text-sm">
          Превышен лимит логопедов для бесплатного плана. Увеличьте план в разделе
          {' '}<a className="underline" href="/settings/billing">Настройки → Подписка</a>.
        </div>
      )}
      ...
      <Card>
        ...
        <div className="space-y-2">
          {pendingExpansions.length === 0 ? (
            <EmptyState title="Нет заявок" description="Измените фильтры сверху и попробуйте снова." />
          ) : (
              ...
              {overLimit.map(({ c, logopedsCount, total }: { c: any; logopedsCount: number; total: number }) => (
                <div key={c.id} className="text-sm flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 whitespace-nowrap">
                    <span><b>{logopedsCount}</b> / {total}</span>
                    <a className="btn btn-ghost btn-xs" href="/settings/billing?quota=logopeds" title="Открыть биллинг">Биллинг</a>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="space-y-4">
        {companies.map((c:any) => {
          const logopedsCount = logopedsByCompany[c.id] || 0
          return (
            <Card key={c.id} className="p-2 sm:p-3">
              ...
              {/* Назначение/снятие руководителя организации */}
              <div className="mt-2 grid gap-2 sm:grid-cols-2 items-end text-xs sm:text-sm">
                <form action={assignOrgManager} className="flex items-end gap-2">
                  <input type="hidden" name="organizationId" value={c.id} />
                  <label className="grid gap-1">
                    <span className="text-[11px]">Email пользователя</span>
                    <input name="userEmail" type="email" className="input w-64" placeholder="user@example.com" required />
                  </label>
                  <button className="btn btn-secondary btn-sm">Назначить руководителя</button>
                </form>
                <form action={revokeOrgManager} className="flex items-end gap-2">
                  <input type="hidden" name="organizationId" value={c.id} />
                  <label className="grid gap-1">
                    <span className="text-[11px]">Email пользователя</span>
                    <input name="userEmail" type="email" className="input w-64" placeholder="user@example.com" required />
                  </label>
                  <button className="btn btn-outline btn-sm">Снять руководителя</button>
                </form>
              </div>
