...
export async function POST(req: Request) {
  try {
    ...
    // Отменяем предыдущие заявки пользователя
    const prev = await prisma.auditLog.findMany({ where: { action: 'PLAN_CHANGE_REQUEST' }, orderBy: { createdAt: 'desc' }, take: 100 })
    for (const r of prev) {
      try {
        const p = JSON.parse((r as any).payload || '{}')
        if (p?.userId === userId) {
          await prisma.auditLog.create({ data: { action: 'PLAN_CHANGE_CANCELED', payload: JSON.stringify({ requestId: r.id, by: 'new_request' }), actorId: userId } })
        }
      } catch {}
    }
...
export async function GET() {
  try {
    ...
    const logs = await prisma.auditLog.findMany({ where: { action: { in: ['PLAN_CHANGE_REQUEST', 'PLAN_CHANGE_CANCELED', 'PLAN_CHANGE_HANDLED'] } }, orderBy: { createdAt: 'desc' }, take: 200 })
    ...
    const lastReq = logs.find(r => {
      if (r.action !== 'PLAN_CHANGE_REQUEST') return false
    ...
    const status = canceled.has(lastReq.id) ? 'canceled' : handled.has(lastReq.id) ? 'handled' : 'pending'
