...
import { enrollChildToLesson,  cancelBooking, cancelEnrollment, generateSlotsNext4Weeks, extendSlots4Weeks, createConsultationRequest, approveConsultationRequest, rejectConsultationRequest, approveParentBooking, rejectParentBooking, createLesson } from './actions'
                    ...
                    <ul className="space-y-1">
                      {dayLessons.map(x => {
                        ...
                        const hadDone = Array.isArray(x.evaluations) && x.evaluations.some((ev) => ev.status === 'DONE')
                          const hasActiveBooking = Array.isArray(x.bookings) && x.bookings.some((b:any)=> b?.status==='ACTIVE')
                          const markerColor = (isPast && hadDone) ? '#FACC15' : (enrolled ? '#10B981' : (hasActiveBooking ? '#3B82F6' : '#EF4444'))
