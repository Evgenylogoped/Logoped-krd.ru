...
import { canUpload, purgeOldMaterialsForLogoped, purgeOldChatForLogoped } from '@/lib/storage'
...
export async function uploadMaterial(formData: FormData): Promise<void> {
  ...
  // Проверка лимитов подписки логопеда перед загрузкой
  const logopedId = (session!.user as any).id as string
  try {
    const check = await canUpload(logopedId, buffer.length)
    if (!check.ok) {
      try { await (prisma as any).auditLog.create({ data: { action: 'PLAN_LIMIT_BLOCK', payload: JSON.stringify({ kind: 'media', logopedId, usedMb: check.usedMb, limitMb: check.limitMb, sizeBytes: buffer.length }) } }) } catch {}
      // Превышение лимита или 0 ГБ — блокируем загрузку
      redirect(`/logoped/child/${childId}?tab=materials&err=storage_limit`)
    }
  } catch {
    // В случае ошибки проверки перестрахуемся и не дадим загрузить
    redirect(`/logoped/child/${childId}?tab=materials&err=storage_limit`)
  }
...
export async function purgeStorageNow(formData: FormData): Promise<void> {
  const session = await getServerSession(authOptions)
  ensureLogoped(session)
  const childId = String(formData.get('childId') || '')
  const logopedId = (session!.user as any).id as string
  try {
    await purgeOldMaterialsForLogoped(logopedId)
    await purgeOldChatForLogoped(logopedId)
  } catch {}
  revalidatePath(`/logoped/child/${childId}?tab=materials`)
  redirect(`/logoped/child/${childId}?tab=materials&saved=1`)
}
