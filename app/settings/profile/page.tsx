...
export default async function SettingsProfilePage({ searchParams }: { searchParams?: Promise<{ saved?: string; photoError?: string; cityError?: string; phoneError?: string; phoneNotSupported?: string }> }) {
  ...
  const sp = (searchParams ? await searchParams : {}) as { saved?: string; photoError?: string; cityError?: string; phoneError?: string; phoneNotSupported?: string }
  return (
    <div className="space-y-6">
      ...
      {sp?.cityError === '1' && (
        <div className="rounded border p-3 bg-amber-50 text-amber-900 text-sm">Выберите город из списка.</div>
      )}
      {sp?.phoneError === '1' && (
        <div className="rounded border p-3 bg-amber-50 text-amber-900 text-sm">Телефон должен быть в формате +7 (XXX) XXX-XX-XX.</div>
      )}
      {sp?.phoneNotSupported === '1' && (
        <div className="rounded border p-3 bg-amber-50 text-amber-900 text-sm">Поле телефона логопеда ещё не добавлено в базу данных. Пожалуйста, примените миграцию Prisma и попробуйте снова.</div>
      )}
      ...
      <section className="section max-w-3xl">
        ...
        <div className="grid gap-3 sm:grid-cols-3 items-start">
          <div className="sm:col-span-2">
            <ChildPhotoUploader childId={userId} action={uploadAvatar} defaultImageUrl={user?.image || null} />
          </div>
      ...
      <section className="section max-w-3xl">
        ...
        <form action={updateTheme} className="grid gap-3">
          <ThemePicker initialTheme={(user?.theme as string | null | undefined) || null} />
          <div>
      ...
      <form action={updateProfile} className="relative grid gap-3 sm:grid-cols-2 max-w-3xl">
        ...
        <label className="grid gap-1">
          ...
          <CityComboBox name="city" required defaultValue={(user?.city as string | undefined) || ''} />
        </label>
        <label className="grid gap-1">
          <span className="text-sm text-muted">Часовой пояс</span>
          <input name="timeZone" className="input" defaultValue={(user?.timeZone as string | undefined) || Intl.DateTimeFormat().resolvedOptions().timeZone} />
        </label>
        <label className="grid gap-1">
          <span className="text-sm text-muted">Телефон</span>
          <PhoneInput
            name="phone"
            defaultValue={isParent ? (parent?.phone || '') : ((user?.phone as string | undefined) || '')}
          />
        </label>
          ...
          <>
            <div className="grid gap-1 sm:col-span-2">
              ...
              <div className="rounded border px-3 py-2 bg-gray-50 text-sm">
                {user?.branch ? (
                  <span>{user.branch.company?.name ? `${user.branch.company.name} — ${user.branch.name}` : user.branch.name}</span>
                ) : (
            ...
            <label className="grid gap-1 sm:col-span-2">
              ...
              <input name="address" className="input" defaultValue={(user?.address as string | undefined) || ''} placeholder="например: ул. Ленина, 10, каб. 5" />
            </label>
            <div className="grid gap-1 sm:grid-cols-2 sm:col-span-2">
              <label className="grid gap-1">
                <span className="text-sm text-muted">Цена занятия (руб.)</span>
                <input name="lessonPrice" type="number" min={0} step={50} className="input" defaultValue={(user?.lessonPrice as number | null | undefined) ?? ''} />
              </label>
              <label className="inline-flex items-center gap-2 mt-6">
                <input type="checkbox" name="showPriceToParents" defaultChecked={Boolean((user as { showPriceToParents?: boolean } | null)?.showPriceToParents)} />
                <span className="text-sm text-muted">Показывать цену клиентам</span>
              </label>
            </div>
            <div className="grid gap-1 sm:grid-cols-2 sm:col-span-2">
              <label className="inline-flex items-center gap-2 mt-1">
                <input type="checkbox" name="isOnline" defaultChecked={Boolean((user as { isOnline?: boolean } | null)?.isOnline)} />
                <span className="text-sm text-muted">Онлайн‑занятия</span>
              </label>
              <label className="inline-flex items-center gap-2 mt-1">
                <input type="checkbox" name="isOffline" defaultChecked={Boolean((user as { isOffline?: boolean } | null)?.isOffline)} />
                <span className="text-sm text-muted">Офлайн‑занятия</span>
              </label>
            </div>
            <label className="grid gap-1">
              <span className="text-sm text-muted">Профессия</span>
              <input name="profession" className="input" defaultValue={(user?.profession as string | undefined) || ''} />
            </label>
            <label className="grid gap-1">
              <span className="text-sm text-muted">Стаж (лет)</span>
              <input name="experienceYears" type="number" min={0} className="input" defaultValue={(user?.experienceYears as number | null | undefined) || ''} />
            </label>
            <label className="grid gap-1 sm:col-span-2">
              <span className="text-sm text-muted">Специализация</span>
              <input name="specialization" className="input" defaultValue={(user?.specialization as string | undefined) || ''} placeholder="Например: ОНР, ЗРР, звукопроизношение" />
            </label>
            <label className="grid gap-1 sm:col-span-2">
              <span className="text-sm text-muted">О себе</span>
              <textarea name="about" className="input min-h-[96px]" defaultValue={(user?.about as string | undefined) || ''} />
              <VisibilityToggles initialShowAbout={!Boolean((user as { hideAboutFromParents?: boolean } | null)?.hideAboutFromParents)} initialShowEducation={!Boolean((user as { hideEducationFromParents?: boolean } | null)?.hideEducationFromParents)} />
            </label>
            <label className="grid gap-1 sm:col-span-2">
              <span className="text-sm text-muted">Образование</span>
              <textarea name="education" className="input min-h-[96px]" defaultValue={(user?.education as string | undefined) || ''} />
            </label>
            {/* VIP метки управляются админами */}
          </>
        )}
