...
export async function uploadChildPhoto(formData: FormData) {
  ...
  try {
    ...
    // delete previous file if stored in our uploads folder
    try {
      if (child?.photoUrl && typeof child.photoUrl === 'string' && child.photoUrl.startsWith('/uploads/children/')) {
        const prev = path.join(process.cwd(), 'public', child.photoUrl)
        await fs.unlink(prev).catch(()=>{})
      }
      // also cleanup other old files for this child id to avoid leftovers
      const files = await fs.readdir(uploadsDir).catch(()=>[] as string[])
      await Promise.all(files.filter(f=> f.startsWith(`${id}_`)).map(f=> fs.unlink(path.join(uploadsDir, f)).catch(()=>{})))
    } catch {}
