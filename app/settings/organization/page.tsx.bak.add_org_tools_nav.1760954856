...
export default async function SettingsOrganizationPage({ searchParams }: { searchParams?: Promise<{ period?: string; saved?: string }> }) {
  ...
  let me = await prisma.user.findUnique({ where: { id: selfId }, include: { branch: { include: { company: { include: { branches: true, owner: true } } } } } })
  // авто-сброс организации, если истёк grace период
  if (me?.orgGraceUntil && new Date(me.orgGraceUntil).getTime() < Date.now()) {
    await prisma.user.update({ where: { id: selfId }, data: { branchId: null, orgGraceUntil: null } })
    me = await prisma.user.findUnique({ where: { id: selfId }, include: { branch: { include: { company: { include: { branches: true } } } } } })
  }
  ...
  const companyBranches = companyId ? await prisma.branch.findMany({ where: { companyId }, include: { users: true, manager: true } }) : []
  const myBranchUsers = me?.branchId ? await prisma.user.findMany({ where: { branchId: me.branchId, role: 'LOGOPED' }, orderBy: { name: 'asc' } }) : []
  const incomingMemberships = session?.user?.email
    ? await prisma.organizationMembershipRequest.findMany({
    ...
    : []
  const companyUsers = companyId ? await prisma.user.findMany({ where: { branch: { companyId } }, select: { id: true, name: true, email: true, role: true, branchId: true, image: true, phone: true, lessonPrice: true, showPriceToParents: true, isOnline: true, isOffline: true, address: true, profession: true, experienceYears: true, specialization: true, about: true, education: true, hideAboutFromParents: true, hideEducationFromParents: true, featured: true, featuredSuper: true, branch: true }, orderBy: [{ branchId: 'asc' }, { name: 'asc' }] }) : []
  const userIds = companyUsers.map(u=>u.id)
  const subs = userIds.length ? await prisma.subscription.findMany({ where: { userId: { in: userIds } }, orderBy: { createdAt: 'desc' } }) : []
  const planMap = new Map<string, string>()
  for (const sub of subs) if (!planMap.has(sub.userId)) planMap.set(sub.userId, String(sub.plan||'').toUpperCase())
  const companyUsersEnriched = companyUsers.map(u => ({ ...u, plan: planMap.get(u.id) || null }))// Параметры статистики
  ...
  return (
    <div className="space-y-6">
      ...
      {companyId && (
        (()=>{
          const allowedBranches = Number(((me?.branch?.company as { allowedBranches?: number } | undefined)?.allowedBranches) ?? 0)
          const totalBranches = (me?.branch?.company?.branches || []).filter((b)=> String(b.name||'').toLowerCase() !== 'основной офис').length
          const allowedLogopeds = Number(((me?.branch?.company as { allowedLogopeds?: number } | undefined)?.allowedLogopeds) ?? 0)
          const totalLogopeds = companyUsers.filter((u)=> u.role==='LOGOPED').length
          const banners: { kind: 'branches'|'logopeds'; text: string }[] = []
          if (totalBranches >= allowedBranches && allowedBranches > 0) banners.push({ kind:'branches', text:`Достигнут лимит филиалов (${totalBranches} из ${allowedBranches}).` })
          if (totalLogopeds >= allowedLogopeds && allowedLogopeds > 0) banners.push({ kind:'logopeds', text:`Достигнут лимит логопедов (${totalLogopeds} из ${allowedLogopeds}).` })
          return banners.length ? (
            <div className="space-y-2">
              {banners.map(b=> (
                <div key={b.kind} className="rounded border p-3 bg-amber-50 text-amber-900 text-sm flex items-center justify-between">
                  <div>{b.text} Перейдите в раздел <a className="underline" href="/settings/billing">Подписка</a> для увеличения лимитов.</div>
                  <a className="btn btn-ghost btn-xs" href={`/settings/billing?quota=${b.kind}`}>Открыть биллинг</a>
                </div>
              ))}
            </div>
          ) : null
      ...
      <section className="section">
        {companyId ? (
          <div className="rounded border p-3 mb-4 bg-slate-50">
            ...
            <div className="text-sm">Филиалов: <span className="font-semibold">{branches.length}</span> из <span className="font-semibold">{Number(((me?.branch?.company as { allowedBranches?: number }|undefined)?.allowedBranches) ?? 0)}</span></div>
              ...
              {companyBranches.map((b)=> (
                <div key={b.id}>
                  ...
                  <ul className="space-y-2 mt-1">
                    {companyUsersEnriched.filter((u)=> u.branchId===b.id).length===0 && (
                      <li className="text-xs text-muted">Нет логопедов</li>
                    )}
                    {companyUsersEnriched.filter((u)=> u.branchId===b.id).map((u)=> (
                      <li key={u.id} className="p-3 text-sm rounded-md border shadow-sm" style={{ background: 'var(--card-bg)' }}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="flex items-center gap-2">
                              <LogopedPreviewTrigger name={u.name || u.email} image={(u as any).image} phone={(u as any).phone} subscription={(u as any).plan || (u as any).subscription || (u as any).subscriptionPlan || null} isOnline={(u as any).isOnline} isOffline={(u as any).isOffline} lessonPrice={(u as any).lessonPrice} showPriceToParents={(u as any).showPriceToParents} address={(u as any).address} profession={(u as any).profession} experienceYears={(u as any).experienceYears} specialization={(u as any).specialization} about={(u as any).about} education={(u as any).education} hideAboutFromParents={(u as any).hideAboutFromParents} hideEducationFromParents={(u as any).hideEducationFromParents} >
                <img src={(u as any).image || '/avatar-user.svg'} alt="" className="h-6 w-6 rounded object-cover" />
                  <span className="underline cursor-pointer">{u.name || u.email}</span>
                <span title="Профиль" className="text-muted">ℹ︎</span>
              </LogopedPreviewTrigger>
                              {String(u.role)==='LOGOPED' && (((u as { featuredSuper?: boolean }).featuredSuper) || ((u as { featured?: boolean }).featured)) && (
                                <VipBadge level={(u as { featuredSuper?: boolean }).featuredSuper ? 'VIP+' : 'VIP'} />
                              )}
                            </span>
                            {((u as any).plan || (u as any).subscription || (u as any).subscriptionPlan) && (<span className="badge">{(u as any).subscription || (u as any).plan || (u as any).subscriptionPlan}</span>)}
          ...
          <>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded border p-3">
                ...
                <div className="text-xs text-muted mt-2">Доступно: {branches.length} из {Number(((me?.branch?.company as { allowedBranches?: number } | undefined)?.allowedBranches) ?? 0)}</div>
              </div>
              <div className="rounded border p-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded border p-3">
                ...
                <ul className="space-y-2">
                  {myBranchUsers.filter((u: { id: number }) => u.id !== selfId).length === 0 && <li className="text-sm text-muted">Нет подчинённых</li>}
                  {myBranchUsers
                    .filter((u: { id: number }) => u.id !== selfId)
                    .sort((a: { featuredSuper?: boolean; featured?: boolean }, b: { featuredSuper?: boolean; featured?: boolean }) => (b.featuredSuper ? 2 : b.featured ? 1 : 0) - (a.featuredSuper ? 2 : a.featured ? 1 : 0))
                    .map((u: { id: number; name?: string; email?: string; role?: string; featuredSuper?: boolean; featured?: boolean }) => (
                    <li key={u.id} className="p-3 text-sm rounded-md border shadow-sm" style={{ background: 'var(--card-bg)' }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="flex items-center gap-2">
                            {u.name || u.email}
                            {u.role === 'LOGOPED' && (u.featuredSuper || u.featured) && (
                              <VipBadge level={u.featuredSuper ? 'VIP+' : 'VIP'} />
                            )}
                          </span>
                        </div>
                      </div>
